...
public:
	...
	void Gravity() {
		float g = acceleration.getY()*0.5+20;
		acceleration.setY(g);
		acceleration.setX(acceleration.getX()*0.5);
		printf("%f\n", acceleration.getY());
	}
...
int main(int argc, char* args[])
{
	...
	else
	{
		...
		else
		{
			...
			RownanieProstej pierwsza(1, 0, -700, -20, 800, -20, 800);
			RownanieProstej druga(1, 0, 0, -20, 800, -20, 800);
			RownanieProstej trzecia(0, 1, 0, -20, 800, -20, 800);
			RownanieProstej czwarta(0, 1,-500, -20, 800, -20, 800);
			...
			while (!quit)
			{
				...
				{	
							Ball.acceleration.setX(-Ball.acceleration.getX());
				...
				if (trzecia.wykrycieKolizjiPrawej(Ball.position.x, Ball.position.y))
				{
					...
					Ball.acceleration.setY(-Ball.acceleration.getY());
